...
public class InputActivity extends AppCompatActivity {
    ...
    public final static String EXTRA_CATEGORY = "jp.techacademy.bryan.thogerson.taskapp.CATEGORY";
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(data != null){
            if (requestCode == 0) {
                int categoryId = data.getIntExtra(EXTRA_CATEGORY, -1);
                Realm realm = Realm.getDefaultInstance();
                mCategory = realm.where(Category.class).equalTo("id", categoryId).findFirst();
                realm.close();

                if (mCategory == null) {
                    mCategoryButton.setText("カテゴリを選択");
                } else {
                    mCategoryButton.setText(mCategory.getName());
                }
            }
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mTask == null) {
            ...
            mHour = calendar.get(Calendar.HOUR_OF_DAY);
            mMinute = calendar.get(Calendar.MINUTE);
            mCategoryButton.setText(mTask.getCategory());
    ...
    private void addTask() {
        ...
        if(mCategory.getName()==null){
            mTask.setCategory("なし");
        }else{
            mTask.setCategory(mCategory.getName());
        }
